...
void eruptor::renderer::Renderer::Render_model(resource::Model_handle model_handle)
{
    ...
    std::clog << "Model id: " << model_handle.Get_id() << " meshes_amount: " << model.Meshes_handles.size() << '\n';

    for(auto mesh_handle : model.Meshes_handles)
    {
        auto mesh = hw_resource_manager.Get_mesh( mesh_handle );

        std::clog << "Mesh " << mesh_handle.Get_id()
        << " | indices: " << mesh.indices_amount
        << " | i_offset: " << mesh.indices_offset
        << " | v_offset: " << mesh.vertex_offset << '\n';
